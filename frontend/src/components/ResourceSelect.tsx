...
interface ResourceSelectProps {
  ...
  selectedWave: string[];
  selectedKite: string[];
  selectedCoaxial: string[];
  onToggle: (tech: 'wind' | 'wave' | 'kite' | 'coaxial', path: string) => void;
}

export default function ResourceSelect(props: ResourceSelectProps) {
  const { available, selectedWind, selectedWave, selectedKite, selectedCoaxial, onToggle } = props;

  const resourceGroups: { label: string; key: 'wind' | 'kite' | 'wave' | 'coaxial'; designs: DesignOption[] }[] = [
    { label: 'Wind',    key: 'wind',    designs: available.wind },
    { label: 'Kite',    key: 'kite',    designs: available.kite },
    { label: 'Coaxial', key: 'coaxial', designs: available.coaxial },
  ...
  const totalSelected = selectedWind.length + selectedWave.length + selectedKite.length + selectedCoaxial.length;
    ...
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <MenuButton className="inline-flex justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 ring-1 shadow-xs ring-gray-300 ring-inset hover:bg-gray-50">
          Design Selection{totalSelected > 0 ? ` (${totalSelected} selected)` : ''}
      ...
      <MenuItems
        transition
        className="absolute z-10 mt-2 p-4 origin-top-left rounded-md bg-white ring-1 shadow-lg ring-black/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in w-max max-w-[95vw] max-h-[80vh] overflow-y-auto py-6"
      >
              ...
              <div className="grid grid-cols-3 gap-x-8 gap-y-2 grid-flow-row">
                {designs.map(design => (
                  <div className="flex" key={design.path}>
                    ...
                    />
                    <label htmlFor={design.path} className="text-sm text-gray-700 ms-3 whitespace-nowrap">
                      {design.label}
                    </label>
                  </div>
                ))}
        ...
        {available.wind.length === 0 && available.wave.length === 0 && available.kite.length === 0 && available.coaxial.length === 0 && (
